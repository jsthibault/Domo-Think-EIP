...
class RatedStoreViewController: UIViewController, UISearchResultsUpdating, UITableViewDataSource, UITableViewDelegate  {
    ...
    {
        let cell: StoreTableViewCell = tableView.dequeueReusableCellWithIdentifier("RatedCell", forIndexPath: indexPath) as! StoreTableViewCell
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject!) {
        if (segue.identifier == "showDetails") {
            let svc = segue.destinationViewController as! DescPlugStoreViewController;
            
            if let selectedStoreCell = sender as? StoreTableViewCell {
                let indexPath = pluginListView.indexPathForCell(selectedStoreCell)!
                let selectedStorePlugin: StorePlugin!
                if (resultSeachController.active) {
                    selectedStorePlugin = filteredPlugins[indexPath.row]
                } else {
                    selectedStorePlugin = allPlugins[indexPath.row]
                }
                svc.plugin = selectedStorePlugin
            }
